...
let handleLogin = (email, password) => {
    return new Promise( async (resolve, reject) => {
        try{
            ...
            if(isExist) {
                const [rows, fields] = await pool.execute("SELECT pass FROM khachhang where email = ?", [email]);
...
let checkEmail = (email) => {
    return new Promise( async (resolve, reject) => {
        try{
            const [user, fields] = await pool.execute("SELECT * FROM khachhang where email = ?", [email]);
            if(!user.length)
            {
                resolve(false);
            }
...
let getInfoUser = (email) => {
    return new Promise( async (resolve, reject) => {
        try{
            let data = {};
            const [rows] = await pool.execute("SELECT * FROM khachhang where email = ?", [email]);
            const id_User = rows[0].id;      
            const [user] = await pool.execute("SELECT * FROM khachhang where id = ?", [id_User]);
